...
export default function AttendanceScannerModal({ open, onClose, date, onRecorded, actorName, activeGroupId, groups }) {
  ...
  const { byId: studentsById, isLoading: studentsLoading } = useStudentsIndex();
  const now = new Date();
  ...
  const fetchAndRecord = async (studentId) => {
    try {
      const { data: student } = await withTimeout(
        supabase.from('students').select('id, name, student_number, group_id').eq('id', studentId).single(),
        NETWORK_TIMEOUT_MS
      );
      if (student) {
  ...
  const handleDecoded = (text) => {
    ...
    lastScanRef.current[studentId] = scanTime;

    const cachedStudent = studentsById.get(studentId);
    if (cachedStudent) {
      recordAttendanceForStudent(cachedStudent); // ⚡ المسار الشائع: تعرّف فوري من الكاش المحلي
    } else {
      fetchAndRecord(studentId); // نادر: طالب جديد لسه ما وصلش الكاش، نحاول من السيرفر
    }
  };
